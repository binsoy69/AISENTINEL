...
Examples:
  ...
  python3 front_node_cellphone_cheat_pi.py --confidence 0.4
    ...
    parser.add_argument("--confidence", type=float, default=None,
                        help="Base detection confidence (default: config)")
    args = parser.parse_args()
    config = load_test_config(args.config, DEFAULT_VIDEO_CONFIG_PATH)
    apply_object_config(sys.modules[__name__], config)
    ...
    model_arg = pi_ui.select_object_model(object_model_arg_value)
    if not model_arg:
