...
const EDITABLE_DASHBOARD = true;
const LIVE_PREVIEW_STORAGE_KEY = "aisentinel.editable.livePreviewHidden";
const DEFAULT_EDITABLE_TYPES = ["Using Phone", "Cheat Sheet", "Head Tilt", "Hands Under the Table", "Passing Papers", "Noise Threshold Exceeded"];
...
function readStoredLivePreviewHidden() {
    try {
        const stored = window.localStorage?.getItem(LIVE_PREVIEW_STORAGE_KEY);
...
function sessionIncidents() {
    const session = currentSession();
    if (state.editableMode && session) return state.editableDrafts.filter((incident) => incident.session_id === session.session_id);
...
function editableDraftKey(incident) {
    return String(incident?._client_id || incident?.incident_id || "");
}
...
function displayTimeFromDatetimeLocal(value) {
    const parsed = new Date(String(value || ""));
    return Number.isNaN(parsed.getTime()) ? "" : formatTime(parsed);
}

function parseEditableSeatNumbers(value) {
    return String(value || "").split(/[;,]/).map((item) => Number.parseInt(item.trim(), 10)).filter((item, index, items) => Number.isFinite(item) && item > 0 && items.indexOf(item) === index);
}

function editableSeatInputValue(incident) {
    return (Array.isArray(incident?.student_numbers) ? incident.student_numbers : []).map((seat) => String(seat).padStart(2, "0")).join(", ");
}

function behaviorTypeFromLabel(label) {
    const normalized = String(label || "").toLowerCase();
    if (normalized.includes("noise")) return "noise";
    if (normalized.includes("phone") || normalized.includes("object") || normalized.includes("sheet")) return "object";
    if (normalized.includes("paper") || normalized.includes("passing")) return "passing_papers";
    if (normalized.includes("hand")) return "hands_under_table";
    if (normalized.includes("head")) return "head_behavior";
    return "manual";
}

function normalizeEditableTypeLabel(label) {
    const normalized = String(label || "").trim().toLowerCase();
    const match = DEFAULT_EDITABLE_TYPES.find((type) => type.toLowerCase() === normalized);
    if (match) return match;
    if (normalized.includes("phone")) return "Using Phone";
    if (normalized.includes("sheet")) return "Cheat Sheet";
    if (normalized.includes("head")) return "Head Tilt";
    if (normalized.includes("hand")) return "Hands Under the Table";
    if (normalized.includes("paper") || normalized.includes("passing")) return "Passing Papers";
    if (normalized.includes("noise")) return "Noise Threshold Exceeded";
    return DEFAULT_EDITABLE_TYPES[0];
}

function availableCameraOptions(selectedCamera) {
    const nodes = Array.isArray(state.snapshot.nodes) ? state.snapshot.nodes : [];
    const options = nodes.length ? nodes.map((node) => ({
        node_id: node.node_id || "",
        camera_label: node.camera_label || node.display_name || node.node_id || "Demo Camera",
    })) : [{ node_id: "demo", camera_label: "Demo Camera" }];
    const selected = String(selectedCamera || options[0]?.camera_label || "");
    return options.map((option) => `<option value="${escapeHtml(option.camera_label)}" data-node-id="${escapeHtml(option.node_id)}" ${option.camera_label === selected ? "selected" : ""}>${escapeHtml(option.camera_label)}</option>`).join("");
}

function createEditableDraft(incident = {}) {
    const session = workspaceSession() || currentSession();
    const incidentId = String(incident.incident_id || `incident-manual-${Date.now()}-${++state.editableCounter}`);
    const timestamp = datetimeLocalValue(incident.created_at) || datetimeLocalValue(new Date().toISOString());
    const typeLabel = normalizeEditableTypeLabel(incident.type_label);
    return {
        ...incident,
        incident_id: incidentId,
        session_id: incident.session_id || session?.session_id || "",
        node_id: incident.node_id || "",
        camera_label: incident.camera_label || (Array.isArray(state.snapshot.nodes) && state.snapshot.nodes[0]?.camera_label) || "Demo Camera",
        behavior_type: behaviorTypeFromLabel(typeLabel),
        type_label: typeLabel,
        student_numbers: Array.isArray(incident.student_numbers) ? [...incident.student_numbers] : [],
        created_at: incident.created_at || isoFromDatetimeLocal(timestamp),
        display_time: incident.display_time || displayTimeFromDatetimeLocal(timestamp),
        ...
        _client_id: incident._client_id || incidentId,
        _timestamp_input: timestamp,
        _is_new: !incident.incident_id,
        _file_field: `asset_${incidentId.replace(/[^a-zA-Z0-9_-]/g, "_")}`,
    };
}

function beginEditableMode() {
    if (!currentSession()) {
        showBanner("Create a demo session first, then use the header status to edit evidence.", true);
        return;
    }
    const existingIncidents = workspaceIncidents();
    state.editableMode = true;
    state.editableDrafts = existingIncidents.map((incident) => createEditableDraft(incident));
    state.editableFiles = {};
    state.recordsRenderSignature = "";
    setActiveSection("records");
    setActiveWorkspaceTab("records");
    render();
}

function endEditableMode() {
...
function editableDraftByKey(key) {
    return state.editableDrafts.find((draft) => editableDraftKey(draft) === key) || null;
}

function updateEditableDraftField(key, field, value) {
    const draft = editableDraftByKey(key);
    if (!draft) return;
    if (field === "timestamp") {
        draft._timestamp_input = value;
        draft.created_at = isoFromDatetimeLocal(value);
        draft.display_time = displayTimeFromDatetimeLocal(value);
    } else if (field === "seat_numbers") {
        draft.student_numbers = parseEditableSeatNumbers(value);
    } else if (field === "type_label") {
        draft.type_label = normalizeEditableTypeLabel(value);
        draft.behavior_type = behaviorTypeFromLabel(draft.type_label);
    } else if (field === "camera_label") {
        draft.camera_label = value;
        const node = (Array.isArray(state.snapshot.nodes) ? state.snapshot.nodes : []).find((item) => item.camera_label === value);
        draft.node_id = node?.node_id || draft.node_id;
...
function editableRecordPayload(draft) {
    return {
        incident_id: draft.incident_id,
        session_id: draft.session_id || currentSessionId(),
        timestamp: draft._timestamp_input || datetimeLocalValue(draft.created_at),
        student_numbers: draft.student_numbers || [],
        type_label: normalizeEditableTypeLabel(draft.type_label),
        behavior_type: draft.behavior_type || behaviorTypeFromLabel(normalizeEditableTypeLabel(draft.type_label)),
        camera_label: draft.camera_label || "",
        node_id: draft.node_id || "",
        review_status: draft.review_status || "unverified",
        file_field: state.editableFiles[editableDraftKey(draft)] ? draft._file_field : "",
    };
}
...
function streamEmptyMessage(node) {
    ...
    if (EDITABLE_DASHBOARD) return "Editable demo mode keeps camera previews disconnected.";
...
function recordsRenderSignature(items, selectionMissing, session, totalRecords, pageCount) {
    return JSON.stringify({
        ...
        editable_mode: state.editableMode,
...
function ensureEditableControls() {
    if (els.editableAddRecordButton || !els.recordsClear?.parentElement) return;
    const button = document.createElement("button");
    button.className = "ghost-button editable-add-record-button";
    button.type = "button";
    button.hidden = true;
    button.dataset.editableAddRecord = "true";
    button.textContent = "Add Evidence";
    els.recordsClear.parentElement.insertBefore(button, els.recordsClear);
    els.editableAddRecordButton = button;
}

function renderEditableControls() {
    ensureEditableControls();
    if (els.editableAddRecordButton) {
        els.editableAddRecordButton.hidden = !state.editableMode;
        els.editableAddRecordButton.disabled = !state.editableMode || !currentSession();
    }
    if (state.editableMode) {
        els.recordsFilter.disabled = true;
        els.recordsSearch.disabled = true;
        els.recordsClear.disabled = true;
    }
}

function addEditableRecord() {
    if (!state.editableMode || !currentSession()) return;
    state.editableDrafts.unshift(createEditableDraft());
    resetRecordsPagination();
    state.recordsRenderSignature = "";
    render();
}

function renderEditableEvidenceCell(draft) {
    const key = editableDraftKey(draft);
    const selectedFile = state.editableFiles[key]?.name || "";
    return `
        <div class="editable-evidence-cell">
            ${evidenceCellMarkup(draft)}
            <label class="editable-file-control">
                <span>${escapeHtml(selectedFile || "Upload media")}</span>
                <input type="file" accept="image/png,image/jpeg,image/gif,.gif" data-editable-file="${escapeHtml(key)}">
            </label>
        </div>
    `;
}

function editableTypeOptions(selectedType) {
    const selected = DEFAULT_EDITABLE_TYPES.includes(selectedType) ? selectedType : DEFAULT_EDITABLE_TYPES[0];
    return DEFAULT_EDITABLE_TYPES.map((type) => `<option value="${escapeHtml(type)}" ${type === selected ? "selected" : ""}>${escapeHtml(type)}</option>`).join("");
}

function renderEditableRecords({ selectionMissing, session, totalRecords, pageCount, pageItems }) {
    els.recordsPaginationSummary.textContent = recordsPaginationSummaryText({
        selectionMissing,
        filteredCount: state.editableDrafts.length,
        totalCount: totalRecords,
        pageItems,
    });
    els.recordsPageLabel.textContent = `Page ${state.recordsPage} of ${pageCount}`;
    els.recordsPrevPage.disabled = selectionMissing || !state.editableDrafts.length || state.recordsPage <= 1;
    els.recordsNextPage.disabled = selectionMissing || !state.editableDrafts.length || state.recordsPage >= pageCount;
    els.recordsContextLabel.textContent = session
        ? "Editing demo evidence. Add rows, adjust fields, upload media, then save from the header."
        : "Create a demo session before adding editable evidence.";
    els.recordsBody.innerHTML = pageItems.length ? pageItems.map((draft) => {
        const key = editableDraftKey(draft);
        return `
            <tr class="editable-record-row">
                <td><input class="editable-table-input" type="datetime-local" value="${escapeHtml(draft._timestamp_input || "")}" data-editable-field="timestamp" data-editable-id="${escapeHtml(key)}"></td>
                <td><input class="editable-table-input editable-seat-input" type="text" value="${escapeHtml(editableSeatInputValue(draft))}" placeholder="05" data-editable-field="seat_numbers" data-editable-id="${escapeHtml(key)}"></td>
                <td><select class="records-select editable-table-select" data-editable-field="type_label" data-editable-id="${escapeHtml(key)}">${editableTypeOptions(draft.type_label)}</select></td>
                <td><select class="records-select editable-table-select" data-editable-field="camera_label" data-editable-id="${escapeHtml(key)}">${availableCameraOptions(draft.camera_label)}</select></td>
                <td>${renderEditableEvidenceCell(draft)}</td>
                <td>
                    <div class="editable-row-actions">
                        <select class="review-select ${reviewMeta(draft.review_status).className}" data-editable-field="review_status" data-editable-id="${escapeHtml(key)}">${reviewOptions(draft.review_status)}</select>
                        <button class="ghost-button ghost-button-tight editable-delete-button" type="button" data-editable-delete="${escapeHtml(key)}">Delete</button>
                    </div>
                </td>
            </tr>
        `;
    }).join("") : `<tr><td class="table-empty" colspan="6">${escapeHtml(selectionMissing ? "Create a demo session first to add editable records." : "No editable evidence yet. Use Add Evidence to create the first row.")}</td></tr>`;
}
...
function renderSessionSummary() {
    ...
    els.statusPill.textContent = state.editableMode ? "Save" : (session ? status : "Idle");
    els.statusPill.className = `status-pill ${state.editableMode ? "is-active editable-save-pill" : `${statusClass} editable-status-control`}`;
    els.statusPill.setAttribute("role", "button");
    els.statusPill.setAttribute("tabindex", "0");
    els.statusPill.setAttribute("aria-label", state.editableMode ? "Save editable evidence records" : "Edit evidence records");
...
function renderMetrics() {
    ...
    els.metricOnlineFoot.textContent = nodes.length
        ? "Demo nodes are simulated online; camera previews stay hidden until manually shown."
...
function renderRecords() {
    ...
    if (state.editableMode) {
        const editablePageCount = clampRecordsPagination(state.editableDrafts.length);
        const editablePageItems = recordsPageItems(state.editableDrafts);
        renderEditableRecords({
            selectionMissing,
            session,
            totalRecords,
            pageCount: editablePageCount,
            pageItems: editablePageItems,
        });
        return;
    }
...
async function fetchForm(url, formData, options = {}) {
    const response = await fetch(url, { method: "POST", body: formData, ...options });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload.error || payload.message || `Request failed (${response.status})`);
    return payload;
}
...
async function clearRecords() {
    if (state.editableMode) {
        showBanner("Save or leave edit mode before clearing records.", true);
        return;
    }
    const session = workspaceSession();
    if (!session) {
...
async function deleteSession(sessionId) {
    if (state.editableMode) {
        showBanner("Save editable evidence before deleting sessions.", true);
        return;
    }
    const targetId = String(sessionId || "").trim();
    if (!targetId) return;
...
async function deleteSubject(subjectCode) {
    if (state.editableMode) {
        showBanner("Save editable evidence before deleting subject history.", true);
        return;
    }
...
async function saveEditableChanges() {
    if (!state.editableMode || state.editableSaving) return;
    if (!currentSession()) {
        endEditableMode();
        render();
        showBanner("Create a demo session before saving evidence.", true);
        return;
    }
    state.editableSaving = true;
    try {
        const formData = new FormData();
        const records = state.editableDrafts.map((draft) => editableRecordPayload(draft));
        formData.append("records", JSON.stringify(records));
        for (const draft of state.editableDrafts) {
            const key = editableDraftKey(draft);
            const file = state.editableFiles[key];
            if (file) formData.append(draft._file_field, file, file.name);
        }
        const result = await fetchForm("/api/v1/editable/incidents", formData);
        const sessionId = currentSessionId();
        if (sessionId && Array.isArray(result.incidents)) {
            state.incidentsBySession[sessionId] = result.incidents;
            rememberIncidentList(result.incidents);
        }
        endEditableMode();
        showBanner(`Saved ${Number(result.saved_count || 0)} editable evidence record(s).`);
        await refresh();
    } finally {
        state.editableSaving = false;
        render();
    }
}

async function deleteEditableIncident(key) {
    const draft = editableDraftByKey(key);
    if (!draft) return;
    const label = draft.type_label || "this evidence";
    if (!window.confirm(`Delete ${label}? This removes the demo record and its uploaded media.`)) return;
    if (draft._is_new) {
        state.editableDrafts = state.editableDrafts.filter((item) => editableDraftKey(item) !== key);
        delete state.editableFiles[key];
        state.recordsRenderSignature = "";
        render();
        return;
    }
    const result = await fetchJson(`/api/v1/editable/incidents/${encodeURIComponent(draft.incident_id)}`, { method: "DELETE" });
    const sessionId = draft.session_id || currentSessionId();
    if (sessionId && Array.isArray(result.incidents)) {
        state.incidentsBySession[sessionId] = result.incidents;
        rememberIncidentList(result.incidents);
    }
    ...
    delete state.editableFiles[key];
    state.recordsRenderSignature = "";
    showBanner("Deleted editable evidence.");
    render();
}
...
els.statusPill.addEventListener("click", () => {
    if (state.editableMode) {
        saveEditableChanges().catch((error) => showBanner(error.message, true));
    } else {
        beginEditableMode();
    }
});
...
document.addEventListener("click", (event) => {
    ...
    const addRecordButton = event.target.closest("[data-editable-add-record]");
    if (addRecordButton) return addEditableRecord();
    const deleteEditableButton = event.target.closest("[data-editable-delete]");
    if (deleteEditableButton) return deleteEditableIncident(deleteEditableButton.dataset.editableDelete).catch((error) => showBanner(error.message, true));
...
document.addEventListener("input", (event) => {
    const editableField = event.target.closest("[data-editable-field]");
    if (!editableField || !state.editableMode) return;
    updateEditableDraftField(editableField.dataset.editableId, editableField.dataset.editableField, editableField.value);
});

document.addEventListener("change", (event) => {
    const editableFile = event.target.closest("[data-editable-file]");
    if (editableFile && state.editableMode) {
        const file = editableFile.files?.[0] || null;
        if (file) state.editableFiles[editableFile.dataset.editableFile] = file;
        else delete state.editableFiles[editableFile.dataset.editableFile];
        state.recordsRenderSignature = "";
        renderRecords();
        return;
    }
    const editableField = event.target.closest("[data-editable-field]");
    if (editableField && state.editableMode) {
        updateEditableDraftField(editableField.dataset.editableId, editableField.dataset.editableField, editableField.value);
        state.recordsRenderSignature = "";
        renderRecords();
        return;
...
document.addEventListener("keydown", (event) => {
    ...
    if ((event.key === "Enter" || event.key === " ") && event.target === els.statusPill) {
        event.preventDefault();
        if (state.editableMode) saveEditableChanges().catch((error) => showBanner(error.message, true));
        else beginEditableMode();
    }
});
