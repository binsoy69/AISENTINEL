...
Examples:
  ...
  python runtime/front_node_pi/main_video.py --object-model models/custom.hef
    ...
    parser.add_argument("--video", default=None, help="Optional video path. If omitted, the web setup form or config value is used.")
...
def _resolve_dashboard_video_path(session_details, requested_video, config) -> Path | None:
    return (
        resolve_cli_path(session_details.get("video_path"))
        or requested_video
        or config.default_video
    )


def _resolve_calibration_path(args, session_details, config, setup_io, video_path, head_mod):
    ...
    if explicit_calibration is not None:
        ...
        if config.video_source.default_setup_profile is not None:
            if config.video_source.default_setup_profile.exists():
                ...
                head_mod.log_info(
                    "Configured setup profile not found. Falling back to auto/manual setup."
                )
...
def main() -> None:
    ...
    requested_calibration = resolve_cli_path(args.calibration_file)
    ...
    print("  Flow            : web login -> session setup -> video playback")
    print("=" * 78)
    print()
    ...
    def run_monitoring_session(session_details):
        ...
        try:
            ...
            if video_path is None:
                raise RuntimeError(
                    "Select a video file in session setup or set video_source.default_video in config_video.ini."
                )
            if not video_path.is_file():
                raise RuntimeError(f"Video file not found: {video_path}")

            combined_mod.update_dashboard_context(
                source_label=video_path.name,
            )
            combined_mod.set_dashboard_status(
                "starting",
                "Opening video file and loading AI models...",
            ...
            combined_mod.update_dashboard_context(
                setup_profile_path=calibration_path or "",
            )
            ...
            if calibration_path is not None:
                try:
                    ...
                    if explicit_calibration is not None:
                        raise RuntimeError(
                            f"Failed to load setup file: {calibration_path}\n{exc}"
                        ) from exc
                    head_mod.log_info(
            ...
            combined_mod.set_dashboard_status(
                "running",
                "Playing back video through the dashboard.",
            ...
            combined_mod.set_dashboard_status(
                "completed",
                "Video monitoring session ended.",
                monitoring_active=False,
                system_state="idle",
            )
        except Exception as exc:
    ...
    default_video_hint = requested_video or config.default_video
    initial_source_label = (
        default_video_hint.name if default_video_hint is not None else "awaiting_video_selection"
    )

    combined_mod.configure_web_dashboard(
        auth_config=config.web_dashboard,
        runtime_mode="video",
        source_label=initial_source_label,
        config_path=config.config_path,
        evidence_root=config.evidence_root,
        setup_profile_path=requested_calibration or config.video_source.default_setup_profile,
        start_monitoring_callback=start_monitoring_from_dashboard,
        require_session_setup=True,
        session_form_defaults={
            "video_path": str(default_video_hint) if default_video_hint is not None else "",
            "setup_profile_override": str(requested_calibration) if requested_calibration is not None else "",
    ...
    head_mod.log_info("Waiting for login and video session start from the web dashboard...")
